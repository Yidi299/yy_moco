...
from train_hago_4 import CustomModel , CustomModel2
...
def parse_args():
    ...
    parser.add_argument("--ckpt", type=str, default='/data1/liuyidi/scene_cls/V4.1/log_dir/V4.2_linear_try4/ckpt/checkpoint-iter-002000.pyth')
    parser.add_argument("--out", type=str, default='/data1/liuyidi/scene_cls/V4.1/log_dir/V4.2_linear/val')
...
def validate(model, loader, OUT_NAME=None):
    ...
    for cur_iter, (images, target) in enumerate(loader):
        ...
        # #对output_ls进行求平均
        # output = torch.mean(torch.stack(output_ls), dim=0)
        # if hvd.size() > 1:
        #     target = hvd.allgather(target)
...
def main():
    ...
    # model.linear_layer2.data = cls_weight['model_state']['fc.bias'].data.cuda()
