...
def get_pr_xeqy(predict_score, groundtruth_label, taget_labels, taget_precision='x=y', merge_method='sum', lab_w=None):
    ...
    if len(taget_labels) > 1:
        ...
        ss = [(merge_f([s[i] for i in taget_labels]),
               1 if l[0] in taget_labels_set else 0,
               lab_w[l])
    ...
    pr = []
    m = sum([w for s, l, w in ss if l == 1]) #target类别的总数
    n = 0
    t = 0
    for s, l, w in ss:
        if l == 1:
...
def main():
    list_file = '/data1/liuyidi/scene_cls/V3_2_list/V3_all_val.txt'
    ret_file = '/data1/liuyidi/scene_cls/6b_2'
    try:
        log_dir = sys.argv[3]
    ...
    if len(ret_file) >=1: #多个模型结果做集成
        ret = ret_file[0]
        predict_score_all = None
        for ret in ret_file:
            predict_score = np.load('/data1/liuyidi/scene_cls/V3_merge_43/val_online-pr.npy')
            if num_classes is None:
                ...
                for n in num_classes:
                    ...
                    j = j+n
            groundtruth_label = np.load('/data1/liuyidi/scene_cls/V3_merge_43/val_online-lb.npy')
        ...
        a = predict_score_all[-1]
        b = predict_score_all_n[-1] 
    ...
    g = [int(line.strip().split()[1].split('|')[0].split(':')[0]) for line in open(list_file)]
    g2 = np.argmax(predict_score,axis=1).tolist()
    fg = (g==g2)
        ...
        for num_classes_i in range(len(num_classes)):
            ...
            if predict_score.shape[1] <= 100:
                ...
                for i in range(predict_score.shape[1]):
                    #print(i)
                    xeqy, m, n, th = get_pr_xeqy(predict_score, groundtruth_label, taget_labels=[i,i+1,i+2])
                    print(i, xeqy, m, n, th) #####################
                    ret.append([xeqy, m/n])
