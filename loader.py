...
def train_loader_fast(list_file, base_dir, batch_size, threads, hvd, **args):
    size = 224
    if 'size' in args:
        size = args['size']
        del args['size']
    for k in list(args.keys()):
        if not args[k]:
            del args[k]
    def f(x):
        x = random_crop(x, size=size, scale=(0.2, 1), flip_p=0.5)
        '''if np.random.rand() < 0.8:
            if isinstance(x, np.ndarray):
                x = Image.fromarray(np.copy(x))
            x = adjust_hue(x, hue_factor_p=0.1)'''
        ...
        if np.random.rand() < 0.8:
            ...
            x = x.transpose((2, 0, 1))
        return torch.from_numpy(np.copy(x))
    transform = TwoCropsTransform(f)
    dataset = VideoSet(list_file, base_dir, transform=transform, hvd=hvd, reader='TurboJPEG', **args)
    loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
...
def train_diff_cls_loader(list_file, base_dir, batch_size, threads, hvd, **args):
    for k in list(args.keys()):
        if not args[k]:
            del args[k]
    size = 224
    if 'size' in args:
        ...
        del args['size']
    diff_mode = 1
    if 'diff_mode' in args:
        diff_mode = args['diff_mode']
        del args['diff_mode']
    def f(xs, tmeta=None):
        assert len(xs) == 2
        xs = random_crop(xs, size=size, scale=(0.2, 1), flip_p=0.5)
        for i in range(len(xs)):
            if np.random.rand() < 0.5:
                shape0 = xs[0].shape
                p0 = np.random.rand() * 0.8 + 0.2
                p1 = np.random.rand() * 0.8 + 0.2
                xs[i] = cv2.resize(xs[i], (int(shape0[1]*p0), int(shape0[0]*p1)), interpolation=cv2.INTER_LINEAR)
                xs[i] = cv2.resize(xs[i], (shape0[1], shape0[0]), interpolation=cv2.INTER_LINEAR)
        for i in range(len(xs)):
            if np.random.rand() < 0.5:
                quality = random.randint(10, 90)
                _, bs = cv2.imencode('.jpg', xs[i], [int(cv2.IMWRITE_JPEG_QUALITY), quality])
                xs[i] = cv2.imdecode(np.frombuffer(bs, np.uint8), cv2.IMREAD_COLOR)
        if np.random.rand() < 0.5:
            xs = [xs[1], xs[0]]
        xs = [distort_color(x) for x in xs] # to CHW float32
        #xs = distort_color(xs)
        xs = [(np.clip(x, 0.0, 1.0) * 255).astype('uint8') for x in xs]
        return torch.from_numpy(np.concatenate(xs, axis=0))
    def samp_2(names, target):
        if np.random.rand() < 0.2:
            i = random.randint(0, len(names)-1)
            return [names[i], names[i]], 0
        diff = random.randint(3*4, 9*4)
        if diff >= len(names)-1:
            return [names[0], names[-1]], target
        i = random.randint(0, len(names)-1-diff)
        return [names[i], names[i+diff]], target
    dataset = VideoSet(list_file, base_dir, transform=f, hvd=hvd, reader='TurboJPEG',
                       diff_mode=True, diff_mode_samp_2=samp_2, **args)
    loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
...
def val_diff_cls_loader(list_file, base_dir, batch_size, threads, hvd, **args):
    for k in list(args.keys()):
        if not args[k]:
            del args[k]
    size = 256
    ...
    def f(xs, tmeta=None):
        for i in range(len(xs)):
            xs[i] = cv2.resize(xs[i], (size, size), interpolation = cv2.INTER_LINEAR)
        xs = [x.transpose((2, 0, 1)) for x in xs]
        return torch.from_numpy(np.concatenate(xs, axis=0))
    def samp_2(names, target):
        diff = 5*4
        if diff >= len(names)-1:
            return [names[0], names[-1]], target
        i = (len(names)-1-diff) // 2
        return [names[i], names[i+diff]], target
    dataset = VideoSet(list_file, base_dir, transform=f, hvd=hvd, reader='TurboJPEG',
                       diff_mode=True, diff_mode_samp_2=samp_2, **args)
    loader = torch.utils.data.DataLoader(
...
def train_cls_loader_npy(list_file, base_dir, batch_size, threads, hvd, **args):
    size = 224
    if 'size' in args:
        size = args['size']
        del args['size']
    ...
    def f(x):
        x = x.astype('float32')
        ###############################################
        '''hh = random.randint(300, 800)
        if x.shape[0] > hh:
            hi = random.randint(0, x.shape[0] - hh)
        else:
            hi = 0
        x = x[hi:hi+hh, :]
        x = cv2.resize(x, (x.shape[1], x.shape[0]//2), interpolation = cv2.INTER_LINEAR)'''
        ###############################################
        x = random_crop(x, size=size, scale=(0.5, 1))
        SpecAugment(x, 80, 2, 80, 2, 0.5)
        x = x * np.random.uniform(0.5, 1.5)
        x = np.stack([x]*3, axis=2)
        return torch.from_numpy(x)
    dataset = VideoSet(list_file, base_dir, transform=f, hvd=hvd, reader='npy', **args)
    loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
...
def val_cls_loader_npy(list_file, base_dir, batch_size, threads, hvd, **args):
    size = 256
    if 'size' in args:
        size = args['size']
        del args['size']
...
def train_cls_loader_npy_img(list_file, base_dir, batch_size, threads, hvd, **args):
    assert isinstance(base_dir, list) and len(base_dir) == 2
    ...
    dataset = VideoSet(list_file, base_dir, transform=[f_npy, f_img], hvd=hvd, reader=['npy', 'TurboJPEG'], **args)
    loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
...
def val_cls_loader_npy_img(list_file, base_dir, batch_size, threads, hvd, **args):
    assert isinstance(base_dir, list) and len(base_dir) == 2
    size = 256
    if 'size' in args:
        size = args['size']
...
#     list_file = '/data1/liuyidi/scene_cls/V4.1/argu_list/train_list/val_27_1646.txt'
